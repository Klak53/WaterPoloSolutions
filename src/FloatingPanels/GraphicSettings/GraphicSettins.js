...
const GraphicSettings = ({ forwarderRef }) => {
  ...
  const { waves, setWaves, ripples, setRipples, shadows, setShadows } = usePool();

  return (
    <FloatingPanel id="graphicSettings" icon={<FaCog />} title="graphic settings" forwarderRef={forwarderRef} visible={graphicSettings} close={() => setGraphicSettings(false)}>
      ...
      <FloatingPanelFooter>
        <p>This effects are only visible on the screen and as such they are not affecting the screenshots but turning them off can improve performance on the low end devices</p>
      </FloatingPanelFooter>
    </FloatingPanel>
  );
};
